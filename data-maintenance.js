...
(function(){
  ...
  function toast(msg){const el=$('dataMaintenanceStatus');if(el)el.innerHTML=msg;console.log('[ikun data]',msg)}
  function clearCache(){try{IKUN_SESSION_KEYS.forEach(k=>sessionStorage.removeItem(k));Object.keys(sessionStorage).filter(k=>/market|fund|rank|manager|ikun/i.test(k)).forEach(k=>sessionStorage.removeItem(k));toast('已清理行情 / 低频缓存')}catch(e){toast('清理缓存失败：'+e.message)}}
  ...
  async function clearServiceWorkerCache(){try{if('caches' in window){const keys=await caches.keys();await Promise.all(keys.map(k=>caches.delete(k)))}if('serviceWorker' in navigator){const regs=await navigator.serviceWorker.getRegistrations();await Promise.all(regs.map(r=>r.update()))}toast('缓存已清理，请点击 <button class="btn2" onclick="location.reload()">立即刷新页面</button> 以加载最新版本。')}catch(e){toast('清理 Service Worker 失败：'+e.message)}}
  ...
  async function resetAllData(){const txt=prompt('这是高风险操作，会清空本项目自选、持仓、预警、邮箱标识、缓存并注销 Service Worker。请输入“确认重置”继续：');if(txt!=='确认重置'){toast('已取消重置：未输入确认重置');return}try{IKUN_KEYS.forEach(k=>localStorage.removeItem(k));IKUN_SESSION_KEYS.forEach(k=>sessionStorage.removeItem(k));Object.keys(sessionStorage).filter(k=>/market|fund|rank|manager|ikun/i.test(k)).forEach(k=>sessionStorage.removeItem(k));if('caches' in window){const keys=await caches.keys();await Promise.all(keys.map(k=>caches.delete(k)))}if('serviceWorker' in navigator){const regs=await navigator.serviceWorker.getRegistrations();await Promise.all(regs.map(r=>r.unregister()))}toast('已重置本项目本地数据，页面即将刷新');setTimeout(()=>location.reload(),800)}catch(e){toast('重置失败：'+e.message)}}
