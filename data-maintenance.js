// data-maintenance.js - V1.5 本地数据 / 缓存清理工具
(function(){
  ...
  function clearMarketCache(){try{sessionStorage.removeItem('market-lite-v1');Object.keys(sessionStorage).filter(k=>/market|fund|rank|manager/i.test(k)).forEach(k=>sessionStorage.removeItem(k));toast('已清理行情 / 低频缓存')}catch(e){toast('清理缓存失败：'+e.message)}}
  function clearPortfolio(){localPortfolioKeys.forEach(k=>localStorage.removeItem(k));try{window.renderPortfolioUnified&&window.renderPortfolioUnified();window.renderAiSummaryRules&&window.renderAiSummaryRules()}catch(e){}toast('已清理本地持仓、买卖记录和预警')}
  ...
  async function resetAll(){if(!confirm('确定重置所有本地数据？这会清空自选、持仓、预警、邮箱标识和缓存。'))return;try{localStorage.clear();sessionStorage.clear();if('caches' in window){const keys=await caches.keys();await Promise.all(keys.map(k=>caches.delete(k)))}if('serviceWorker' in navigator){const regs=await navigator.serviceWorker.getRegistrations();await Promise.all(regs.map(r=>r.unregister()))}toast('已重置所有本地数据，页面即将刷新');setTimeout(()=>location.reload(),800)}catch(e){toast('重置失败：'+e.message)}}
  function render(){const data=$('data');if(!data)return;const card=data.querySelector('.card');if(!card||$('dataMaintenanceBox'))return;const box=document.createElement('div');box.id='dataMaintenanceBox';box.className='card data-maintenance-box';box.innerHTML='<h2>本地数据 / 缓存维护</h2><p class="note">用于测试和日常维护：清理 sessionStorage、localStorage、Service Worker 缓存和本地邮箱标识。</p><div class="row"><button class="btn2" onclick="clearMarketCacheV15()">清理行情缓存</button><button class="btn2" onclick="clearPortfolioV15()">清理本地持仓</button><button class="btn2" onclick="clearCustomFundsV15()">清理自选基金</button><button class="btn2" onclick="clearSWCacheV15()">清理 Service Worker 缓存</button><button class="btn2" onclick="logoutLocalV15()">注销本地邮箱</button><button class="btn" onclick="resetAllLocalDataV15()">重置所有本地数据</button></div><div id="dataMaintenanceStatus" class="status">未执行清理操作。</div>';card.parentNode.insertBefore(box,card)}
  window.clearMarketCacheV15=clearMarketCache;
  window.clearPortfolioV15=clearPortfolio;
  window.clearCustomFundsV15=clearCustomFunds;
  window.clearSWCacheV15=clearSW;
  window.logoutLocalV15=logoutLocal;
  window.resetAllLocalDataV15=resetAll;
