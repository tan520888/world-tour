...
一个用于基金观察、自选基金、持仓计划、行情资讯、基金对比、预警提醒、净值走势图、PWA 本地安装、规则版 AI 总结和本地数据维护的个人基金看板。

> 数据仅供个人学习与记录参考，不构成投资建议。基金有风险，投资需谨慎。

## 当前版本

**V1.6：将买卖记录编辑从 prompt 弹窗升级为统一 Modal，并为重置所有本地数据加入“确认重置”强确认。**
...
- 买卖记录 Modal 编辑：支持编辑类型、基金代码、金额、成交净值、备注、日期，并可删除单条记录
- 本地数据维护：支持清理行情缓存、本地持仓、自选基金、Service Worker 缓存、本地邮箱标识和重置所有本地数据
- 强确认重置：重置所有本地数据前必须输入“确认重置”
...
- `site.css`：统一样式入口
- `app.js`：核心基金数据和渲染逻辑
- `site-enhancements.js`：主题、行情资讯、快捷筛选、顶部卡片等增强能力
...
- `trade-editor.js`：买卖记录 Modal 编辑与单条删除
- `data-maintenance.js`：一键清缓存 / 清本地数据维护工具
...
- 本地数据维护工具会清理浏览器本机数据，不影响云端，因为当前尚未接入云同步
